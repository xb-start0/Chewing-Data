...
def get_train_utils(opt, model_parameters):
    ...
    spatial_transform = transforms.Compose([
                        ...
                        transform.NormalizeVideo(mean = [0.5, 0.5, 0.5, 0.5], std = [0.5, 0.5, 0.5, 0.5]),])
    ...
    train_data = get_training_data(opt.video_path, opt.event_path, opt.audio_configs,opt.annotation_path / 'chew_train.json',
...
def get_val_utils(opt):
    ...
    #                                            opt.annotation_path / 'chew_val.json', opt.dataset,
    #                                            opt.input_type, opt.file_type,
    #                                            spatial_transform,
    #                                            temporal_transform)
    val_data = get_validation_data(opt.video_path,opt.event_path, opt.audio_configs,
                                               opt.annotation_path / 'chew_test.json', opt.dataset,
                                               opt.input_type, opt.file_type,
                                               spatial_transform,
                                               temporal_transform)
    if opt.distributed:
    ...
    val_loader = torch.utils.data.DataLoader(val_data,
                                            ...
                                            #              opt.n_val_samples),
                                             ...
                                             sampler=val_sampler,
                                             worker_init_fn=worker_init_fn)
    w = [batch for batch in val_loader]
    if opt.is_master_node:
...
def main_worker(index, opt):
    ...
    if opt.is_master_node:
        print(model)
    criterion = L1Loss().to(opt.device)
    opt.no_val = False
    if not opt.no_train:
