...
def video_process(video_file_path, dst_root_path, ext, fps=-1, size=240):
    ...
    name = video_file_path.parts[-2]+'_'+video_file_path.stem
    ...
    ffmpeg_cmd = ['ffmpeg', '-i', str(video_file_path), '-r', str(frame_rate),'-vf', vf_param]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dir_path', default='/media/Storage2/xb/Chew/3D-ResNets-PyTorch/data', type=Path, help='Directory path of videos')
    parser.add_argument(
        '--dst_path',
        default='/media/Storage2/xb/Chew/RViT-main/data_new/jpgs_new',
        type=Path,
    ...
    if args.dataset in ['chew','kinetics', 'mit', 'activitynet']:
        ext = '.mp4'
    else:
        ext = '.avi'
    ...
    for video_dir_path in video_dir_paths:
        for video_file_path in video_dir_path.iterdir():
            video_process(video_file_path,args.dst_path, ext, args.fps, args.size)
